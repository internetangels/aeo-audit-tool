...
st.title("🔍 AI Search Optimization Audit Tool")
st.markdown("Audit your website for AI-readiness and download a professional report.")

url = st.text_input("Enter the website URL to audit (e.g. https://example.com)")
...
if st.button("Run Audit"):
    if url:
        with st.spinner("Auditing website..."):
            ...
            if "error" in results:
                st.error(f"Failed to fetch the page: {results['error']}")
            else:
                score = calculate_score(results)
                recs = generate_recommendations(results)

                st.success(f"AEO Readiness Score: {score}%")
                st.write("### Audit Summary")
                st.json(results)

                pdf_path = generate_detailed_pdf_report(url, score, results, recs)
                with open(pdf_path, "rb") as f:
                    st.download_button(
                        label="📄 Download PDF Report",
                        data=f,
                        file_name="reviewmate_aeo_audit_report.pdf",
                        mime="application/pdf"
                    )
    else:
        st.warning("Please enter a valid URL to audit.")
